...
╔════════════════════════════════════════════════════╗
║  👤  Gheat                                         ║
║  🎯  Systems Engineer & Quant Developer            ║
║  🛠️   Rust · Python · TypeScript · C++             ║
║  🧠  AI Infrastructure & Vector Databases          ║
║  🖥️   19" Bare-Metal Data Center Rack              ║
║  🐧  Arch Linux (btw) on every node               ║
║  🔐  Full-Stack Security Auditor                   ║
║  📡  Tailscale Mesh · Cloudflare · Caddy           ║
╚════════════════════════════════════════════════════╝
```

- 🏦 **First-ever paid technical intern** at a Hedge Fund *(Summer 2025)*
- 🤖 Built **Atlas** — full agentic OS with React 19 + Rust + Tauri 2 + Ollama
- 🎵 Shipped **Wavly** — CUDA-powered AI DJ music similarity engine
- 🚀 Live at **[gheat.net/spaceships](https://gheat.net/spaceships)** — real-time 3D multiplayer combat
- 🔒 Neutralized SQLi, XSS, JWT forgery & IP leaks across self-hosted stack
- 📡 Private data center powering AI workloads, game servers & web apps
...
|:-:|---------|-------------|-------|
| 🤖 | **Atlas** | AI-Driven Desktop Control Panel & Agentic OS | `React 19` `Rust` `Tauri 2` `Ollama` `TypeScript` |
| 🎵 | **Wavly** | High-Precision DJ Music Similarity Engine | `Python` `Demucs` `CUDA` `librosa` `numpy` `scipy` |
| 🚀 | **[Spaceships](https://gheat.net/spaceships)** | Real-Time Multiplayer 3D Space Combat — live now | `Node.js` `WebSockets` `Express` `SQLite` `JWT` |
| 🌌 | **Voidwatch** | Space Exploration & Physics Simulation Game | `C#` `OOP` `Real-Time Physics` |
| 🖥️ | **SimplyServer** | Cross-Platform Remote Server Management GUI | `Python` `Tailscale API` `Linux Admin` |
| 🧠 | **Custom MCP Tools** | AI Tool Calling & Workflow Automation Suite | `Python` `LLMs` `MCP Protocol` |
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│  🏦  HEDGE FUND — Quantitative Software Engineering Intern   [Summer 2025]   │
├──────────────────────────────────────────────────────────────────────────────┤
│  ★  First-ever paid technical intern in the firm's history                   │
│  →  Production OCR pipeline for legacy handwritten financial contracts       │
│  →  High-dimensional vector DB for quantitative financial analysis           │
│  →  Secure SQL cross-referencing of internal transactional ledgers           │
│  →  Digital asset & crypto market data analysis workflows                    │
└──────────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────────┐
│  ⚙️   INDEPENDENT DEVOPS & GAME DEV — Lead Systems Engineer  [2024–Present]  │
├──────────────────────────────────────────────────────────────────────────────┤
│  →  Private 19" bare-metal data center: AI workloads + game servers          │
│  →  Full security audits: SQLi, XSS, JWT forgery, IP leaks — all patched    │
│  →  Custom headless Arch Linux spin-ups with Bash automation scripts         │
│  →  Tailscale mesh + Cloudflare Tunnels + Caddy reverse proxy stack          │
│  →  3D-printed custom cooling & optimized physical rack layouts              │
│  →  Volunteered teaching CS fundamentals & programming to younger students   │
└──────────────────────────────────────────────────────────────────────────────┘
```
